...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.statusbar.showMessage(self.SCHUL_NM)
        ...
        self.pushButton_2_search.clicked.connect(self.selected_school)
    ...
    def selected_school(self):
        ...
        try:
            self.SCHUL_NM = self.listWidget_search.currentItem().text()
        except:
            return
        url += self.SCHUL_NM
        ...
        index = response.find("ATPT_OFCDC_SC_CODE")
        self.ATPT_OFCDC_SC_CODE = response[index + 28 : index + 31]

        index = response.find("SD_SCHUL_CODE")
        self.SD_SCHUL_CODE = response[index + 23 : index + 30]
    ...
    def get_school_list(self):
        ...
        response = requests.get(url + self.lineEdit_search.text()).text
        soup = bs(response, "lxml-xml")
        res = soup.findAll(text=True)
        for i in range(2, len(res)):
            if "고등학교" in res[i] or "중학교" in res[i] or "초등학교" in res[i]:
                if res[i - 2].isdigit():
                    self.listWidget_search.addItem(res[i])
